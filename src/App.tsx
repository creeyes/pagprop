...
export default function App() {
  ...
  const handleSaveProperty = async () => {
    ...
    try {
      ...
      let url = `${API_BASE_URL}/front/api/properties/`;
      let method = 'POST';

      if (!esNuevo && editForm.id) {
        url = `${API_BASE_URL}/front/api/properties/${editForm.id}/`;
        method = 'PUT';
      }
  ...
  const fetchLocations = async (agencyId: string) => {
    try {
      const url = `${API_BASE_URL}/front/api/locations/?agency_id=${agencyId}`;
      const response = await fetch(url);
      if (response.ok) {
        const data = await response.json();
